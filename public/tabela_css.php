...
?>  
	...
	<div class="box-container m-4">
		<div class="box">
			...
			<div class="flex-btn">
				<a href="detalha_job.php?subject=<?php echo  $linha['id_job'];?>" class="btn">Ver Detalhes</a>
			</div>
		</div>
	</div>
...
<div class="container">
	<div class="row">
		<div class="d-flex justify-content-center align-self-center">
			<button <?php echo $anterior ?? 'disabled' ?> class="m-2 <?php $offset == 1 ? 'current-page' : '' ?>" onclick="redirect('<?php echo $search; ?>', <?php echo $limit;?>, <?php echo $anterior;?>)">Anterior</button>
		<?php for($i = $offset - 4; $i <= $offset; $i++): ?>
			<?php if ($i < 1) { continue; } ?>
			<button class="m-2 <?php $offset == $i ? 'current-page' : '' ?>" onclick="redirect('<?php echo $search; ?>', <?php echo $limit;?>, <?php echo $i;?>)"><?php echo ($i) ?></button>
		<?php endfor; ?>
		<?php for($i = $offset + 1; $i < $offset + 5; $i++): ?>
			<?php if ($i > $totalPaginas) { break; } ?>
			<button class="m-2" onclick="redirect('<?php echo $search; ?>', <?php echo $limit;?>, <?php echo $i;?>)"><?php echo ($i) ?></button>
		<?php endfor; ?>
			<button <?php echo $proximo ?? 'disabled' ?> class="m-2 <?php $offset == $totalPaginas ? 'current-page' : '' ?>" onclick="redirect('<?php echo $search; ?>', <?php echo $limit;?>, <?php echo $proximo;?>)">Próxima</button>
		</div>
	</div>
</div>
